...
# for an n-gram
####
tfCombiner <- function(full.tf, ngram=1, outpath="training", n=100) {
  for (x in 1:n) {
    print(x)
...
#    full.tf3 <- compTFCombiner(full.tf3, 3, outpath)
#    if (timing) print(proc.time()-ptm)
...
#    if (timing) print(proc.time()-ptm)
    ...
    full.tf4 <- compTFCombiner(full.tf4, 4, outpath)
    if (timing) print(proc.time()-ptm)
...
compTFCombinerBuild("training", .75)